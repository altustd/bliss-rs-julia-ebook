...
function exit_pressure(Pc::Real, gamma::Real, epsilon::Real)
    g = gamma
    # Area ratio as function of Mach number (supersonic branch)
    A_ratio(Me) = (1/Me) * ((2/(g+1)) * (1 + (g-1)/2 * Me^2))^((g+1)/(2*(g-1)))
    # Newton solve for Me such that A_ratio(Me) = epsilon
    Me = 2.0   # initial guess (supersonic)
    for _ in 1:50
        f  = A_ratio(Me) - epsilon
        df = (A_ratio(Me * 1.001) - f) / (Me * 0.001)
        Me -= f / (df + 1e-12)
        Me = max(Me, 1.001)
    end
